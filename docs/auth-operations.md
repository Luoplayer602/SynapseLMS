...
Migration hiện tại: `20260924_0007` ([danh mục khóa học](./course-catalog.md)). Xác minh email, khôi phục mật khẩu, quản lý phiên và cấu hình Mailpit được mô tả trong [vòng đời tài khoản](./account-lifecycle.md); các phần sau tại [mời thành viên qua email](./membership-invitations.md) và [hồ sơ học viên](./student-profiles.md). Migration không tự xác minh tài khoản cũ hoặc tạo hồ sơ/khóa/cấp độ giả. API/web được rebuild khi sửa mã; database có thể tiếp tục chạy.
